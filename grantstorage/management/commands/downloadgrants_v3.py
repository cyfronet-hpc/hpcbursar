...
from grantstorage.integration.portalclient.v3 import PortalClient
...
class Command(BaseCommand):
    ...
    def convert_grants_to_localmodels(self, portal_grant_lists, portal_allocation_lists):
        ...
        for portal_allocation_list in portal_allocation_lists:
            for portal_allocation in portal_allocation_list:
                ...
                if end + datetime.timedelta(days=1) <= now and status == 'accepted':
                    status = 'finished'
    ...
    def handle(self, *args, **options):
        ...
        self.ms.store_users(users)
        self.ms.store_groups(groups)
        self.ms.store_grants(grants)
        print('done stores')
